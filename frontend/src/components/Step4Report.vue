...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Interviews Conducted:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### Rationale for Selecting Interviewees\n([\s\S]*?)(?=\n---\n|\n### Interview Transcripts)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - Select Parent_601(index 0): As a parent rep...
        if (!headerMatch) {
        ...
        // e.g.: - **Parent_601(index 0)**: As a parent rep...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform markers (very old format), use as entire answer
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > Chinese quotation marks
        if (!quoteMatches) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Try splitting answer by question numbers
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 2. "1. " or "\n1. " (numbered format, legacy compatible)
      let matches = []
      let match
      ...
      // Fallback to numbered format if no match
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Return whole text if no numbering found or only one match
      if (matches.length <= 1) {
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only shown when real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 (## xxx) since section title is shown externally
  ...
  // Clean <br> between block element end and paragraph start
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: maintain incrementing numbers when single <ol> items are separated by paragraphs
