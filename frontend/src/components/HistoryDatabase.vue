<template>
  ...
  >
    <!-- Background: tech grid lines (only shown with projects) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Start Simulation" and Step 5 "Deep Interaction" must be started while running and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
let observer = null
let isAnimating = false  // Animation lock, prevent flicker
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards per current row, center each row
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Skip if target state matches current state
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Short delay before executing, avoid too-fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
