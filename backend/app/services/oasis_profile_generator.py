...
Converts entities from a Zep knowledge graph into Agent Profile format required by the OASIS simulation platform.
...
class OasisProfileGenerator:
    ...
    Converts entities from a Zep knowledge graph into Agent Profiles for OASIS simulation.
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        separately and merge the results. Parallel requests are used for efficiency.
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, faithfully reproducing known real-world information as much as possible.
...
1. bio: Social media bio, approximately 200 characters
2. persona: Detailed persona description (approximately 2000 characters of plain text), which must include:
   ...
   - Positions and viewpoints (attitude toward topics, content likely to provoke or move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (a critical part of the persona: describe this individual's connection to the events, and their existing actions and reactions in those events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institutional/group entity, faithfully reproducing known real-world information as much as possible.
...
1. bio: Official account bio, approximately 200 characters, professional and appropriate
2. persona: Detailed account profile description (approximately 2000 characters of plain text), which must include:
   - Organization basics (official name, nature, founding background, primary functions)
   ...
   - Official stance (position on key topics, approach to handling controversies)
   - Special notes (profile of the represented group, operational habits)
   - Institutional memory (a critical part of the persona: describe this organization's connection to the events, and its existing actions and reactions in those events)
...
5. mbti: MBTI type describing the account's communication style, e.g., ISTJ for formal and conservative
6. country: Country name (e.g., "China", "United States")
...
- Institutional account posts must be consistent with the organization's identity and role"""
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profiles as CSV format (OASIS official format).
