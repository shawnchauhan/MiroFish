...
4. Supports user conversation with autonomous tool invocation during chat
"""
...
class ReportLogger:
    ...
    def __init__(self, report_id: str, user_id: str = None):
        ...
        Args:
            ...
            user_id: User ID, used to determine the user directory
        """
    ...
    def log(
        ...
        Args:
            ...
            details: Details dictionary, untruncated
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log entry to determine when a section is truly finished and to get the full content.
        """
        self.log(
...
4. Return the most comprehensive and in-depth search results
...
3. Help you understand how public sentiment evolved over time
...
This is not LLM simulation -- it calls the real interview endpoint to get original responses from simulated Agents.
...
[When to Use]
- Need to understand event perspectives from different roles (What do students think? What does the media say? What is the official response?)
- Need to collect opinions and positions from multiple parties
- Need real responses from simulated Agents (from the OASIS simulation environment)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert at writing predictive forecast reports, with a "god's-eye view" of the simulated world -- you can observe every Agent's behavior, statements, and interactions within the simulation.

[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables. The simulation's evolution represents a prediction of what could happen in the future. What you are observing is not "experimental data" but a "rehearsal of the future."

[Your Task]
Write a predictive forecast report that answers:
...
[Report Positioning]
- This IS a simulation-based predictive forecast report, revealing "if X happens, what would the future look like"
- Focus on prediction outcomes: event trajectories, group reactions, emergent phenomena, potential risks
- Agent behaviors and statements in the simulation ARE predictions of future human behavior
...
You are an expert at writing predictive forecast reports, and you are currently writing one section of the report.
...
2. [You MUST quote Agents' original statements and behaviors]
   - Agent statements and behaviors are predictions of future human behavior
   - Use block quote format to present these predictions in the report, e.g.:
...
3. [Language consistency - quoted content must match the report language]
   ...
   - Ensure all quoted content is translated into the same language as the report
   - Preserve the original meaning while ensuring natural, fluent expression
...
- Use **bold text**, paragraph breaks, block quotes, and lists to organize content, but no headings

[Correct example]
```
This section analyzes the sentiment propagation patterns of the event. Through in-depth analysis of the simulation data, we found...

**Initial Outbreak Phase**

Social media served as the primary channel for breaking news:

> "Social media contributed 68% of the initial exposure..."
...
- insight_forge: Deep insight analysis; automatically decomposes questions and searches facts and relationships across multiple dimensions
...
Each reply you can do only ONE of the following two things (never both at once):
...
The system will execute the tool and return results to you. You do not need to and must not write tool results yourself.
...
- Fabricating tool results (Observations); all tool results are injected by the system
...
4. [Block quote formatting - must be standalone paragraphs]
...
7. [Emphasis] Do not add any headings! Use **bold text** instead of sub-section titles"""
...
- Write the body text directly; use **bold text** instead of sub-section titles
...
- If you have sufficient information: output section content starting with "Final Answer:" (must cite the text above)
- If you need more information: call a tool to continue searching
═══════════════════════════════════════════════════════════════"""
...
[Response Style]
- Concise and direct; avoid long-winded responses
...
class ReportAgent:
    ...
    1. Planning phase: Analyze simulation requirements, plan the report outline
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    # Third time: fallback handling, truncate to first tool call and force execute
                    logger.warning(
            ...
            # Sufficient tool calls made; LLM output content but without "Final Answer:" prefix
    ...
    def chat(
        ...
        The Agent can autonomously call search tools to answer questions during conversation.
...
class ReportManager:
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if heading duplicates the section title (skip duplicates in first 5 lines)
                if i < 5:
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Skip divider line immediately after heading
                i += 1
                continue
    ...
    def save_report(cls, report: Report) -> None:
        ...
        # Save metadata JSON (include user_id for restart persistence)
        data = report.to_dict()
        with cls._user_registry_lock:
