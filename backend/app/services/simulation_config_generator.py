...
Uses an LLM to automatically generate detailed simulation parameters based on simulation requirements,
document content, and knowledge graph information. Fully automated with no manual parameter tuning needed.
...
class PlatformConfig:
    ...
    popularity_weight: float = 0.3  # Popularity/hotness
    relevance_weight: float = 0.3  # Relevance

    # Viral threshold (number of interactions before triggering spread)
    viral_threshold: int = 10
    
    # Echo chamber effect strength (degree to which similar views cluster together)
...
- **Important**: The example values below are only for reference. You should adjust time periods based on the nature of the event and the characteristics of participants.
  - Example: Student peak hours might be 9-11 PM; media may be active all day; official institutions only during work hours
  - Example: Breaking news might cause late-night discussions, so off_peak_hours can be shortened
...
{{
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent config based on rules (Chinese daily routines)."""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/Professors: active during work hours + evenings, medium frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
