...
def build_graph():
    ...
    try:
        ...
        task_id = task_manager.create_task(f"Build graph: {graph_name}", user_id=uid)
...
def get_graph_data(graph_id: str):
    """
    Get graph data (nodes and edges) - only returns graphs owned by the current user
    """
    try:
        uid = get_current_user_id()
...
def delete_graph(graph_id: str):
    """
    Delete Zep graph - only the graph owner may delete
    """
    try:
        uid = get_current_user_id()
        if not ProjectManager.find_project_by_graph_id(uid, graph_id):
